...
function Inventory() {
  ...
  const fetchProductsData = () => {
    fetch(`http://localhost:4000/api/product/get/${authContext.user}`)
      ...
      .then((data) => {
        // Set products and generate serial numbers mapping
        const serialNumbers = {};
        data.forEach((product, index) => {
          serialNumbers[index + 1] = product; // Serial numbers start from 1
        });
        setAllProducts(data);
        authContext.setSerialNumbersMap(serialNumbers);
        setSerialNumbersMap(serialNumbers);
      })
        ...
        <div className="overflow-x-auto rounded-lg border bg-white border-gray-200 ">
          ...
          <table className="min-w-full divide-y-2 divide-gray-200 text-sm">
            ...
            <tbody className="divide-y divide-gray-200">
              {Object.keys(serialNumbersMap).map((serialNumber) => {
                const product = serialNumbersMap[serialNumber];
                return (
                  <tr key={product._id}>
